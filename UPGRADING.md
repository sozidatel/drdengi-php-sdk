...
Подготовленные операции добавляют новый путь записи без изменения существующих
сигнатур `create*()`, `RecordWriteToken`, `WriteResult` или исключений.
Мигрировать текущие вызовы не требуется. Новых зависимостей нет.

Для предварительного просмотра или сохранения перед отправкой используй
соответствующий `prepare*()`, затем `submit($prepared)`. Объект сохраняется через
`toJson()` и восстанавливается через `PreparedRecordWrite::fromJson()`.
Отправка не перечитывает валюты и не пересчитывает сохранённые суммы и даты;
клиент того же аккаунта и актуальность справочников обеспечивает приложение.
JSON не содержит credentials, подтверждения записи или разрешения на повтор.

Прежние ограничения серверной дедупликации и запрет автоматического повтора
неоднозначной записи сохраняются. Контракт и пример:
[подготовленные операции](docs/prepared-records.md).

## С 0.7.1 на 0.8.0

Теперь требуется PHP-расширение `ext-curl`. Версии PHP и остальных зависимостей
не меняются.

При ненулевом `ClientOptions::readTimeout` SOAP XML по-прежнему обрабатывает
native `SoapClient`, а HTTP выполняется через cURL. Таймаут ограничивает всё
время одного SOAP HTTP-запроса, включая подключение; дробные секунды округляются
вверх до миллисекунды. Подключение дополнительно ограничивается
`connectTimeout`. WSDL, справочники и failover-вызовы не входят в единый общий
бюджет. Настройка PHP `default_socket_timeout` не изменяется.

Явный raw `stream_context.http.timeout` имеет приоритет. Если в raw context
заданы только TLS-параметры или заголовки, применяется типизированный лимит;
контекст вызывающего кода не изменяется.

Новый HTTP-слой имеет явно ограниченный набор настроек и не выполняет redirects,
чтобы автоматически не пересылать запись и credentials по другому адресу.
Неподдерживаемые настройки отклоняются до сетевого запроса. Для нестандартных
native SOAP options можно явно сохранить прежний транспорт:

```php
new ClientOptions(readTimeout: null)
```

При этом raw `http.timeout` также должен отсутствовать: если он задан, он
включает cURL-транспорт. В native-режиме ожидание SOAP-ответа снова зависит от
`default_socket_timeout`. Полный контракт: [docs/transport.md](docs/transport.md).

## С 0.7.0 на 0.7.1

Сигнатуры существующих методов сохранены. Типизированные `records()->create*()`
теперь отклоняют неполные и некорректные подтверждения записи через
`UnconfirmedWriteException`. Это подкласс `AmbiguousMutationException`, поэтому
существующий обработчик неоднозначных записей продолжает работать. Исключение
содержит `payloads` и исходный `response` для сверки с журналом. Его
`retrySafe=false`, а `endpoint=null`: сервис не получает адрес фактически
ответившего транспорта. Не повторяй запись автоматически; сначала сверь её
результат чтением.

Для legacy-ответов без `client_id` проверяются количество и уникальность
server ID, но соответствие конкретным строкам запроса не выдумывается:
`serverIdForClientId()` остаётся `null`. Обновления по известному `server_id`
по-прежнему допускают пустой legacy-ответ, а низкоуровневый `savePayloads()`
сохраняет прежнее поведение. Подтверждение ID само по себе не заменяет проверку
сохранённых полей чтением.

HTML и повреждённый XML вместо SOAP-ответа теперь также могут приводить к
`AmbiguousMutationException` для записи и переключению endpoint для чтения.
Обычные SOAP business faults сохраняют прежнюю классификацию.

Также исправлены остатки при исключении долгов, скрытых и нулевых счетах,
переполнение сумм при чтении, независимость клонированных отчётных запросов и
сохранение сортировки и экранированного текста в справочниках.

Известное ограничение остаётся: `ClientOptions::readTimeout` пока не ограничивает
ожидание SOAP-ответа native `SoapClient`. Оно зависит от
`default_socket_timeout` окружения. SDK не меняет глобальную настройку PHP;
реализация отдельного HTTP-транспорта в эти исправления не входит.
