...
Изменения релизов перечислены в [CHANGELOG.md](CHANGELOG.md), а важные шаги
миграции — в [UPGRADING.md](UPGRADING.md). Политика совместимости описана в
...
Требуются PHP 8.2+ и расширения `soap`, `curl`, `json`.
...
Unit-тесты не обращаются к реальному аккаунту. Проверки транспорта запускают
временный SOAP-сервер на `127.0.0.1`; для них нужны `proc_open` и разрешённые
...
всегда проходил через классификацию исключений SDK. SOAP-кодек остаётся штатным
`SoapClient`, а при включённом `readTimeout` HTTP-запрос выполняется через cURL.

`readTimeout` ограничивает общее время одного SOAP HTTP-запроса, включая
подключение, отправку и получение ответа. Дробные секунды округляются вверх до
миллисекунды; `connectTimeout` дополнительно ограничивает подключение. Это не
общий бюджет вызова SDK: загрузка WSDL, получение справочников и попытки чтения
на резервных серверах выполняются отдельно. Глобальная настройка PHP
`default_socket_timeout` не меняется.

Raw `connection_timeout` и `stream_context.http.timeout` имеют приоритет над
соответствующими типизированными настройками. Контекст только с TLS-настройками
или заголовками сохраняет типизированный таймаут и не изменяется на месте.
Поддерживаемые HTTP-настройки и ограничения описаны в
[контракте транспорта](docs/transport.md).

`readTimeout: null` без raw `http.timeout` явно возвращает прежний native
HTTP-транспорт со всеми его SOAP options. В этом режиме время ожидания зависит
от `default_socket_timeout` окружения. `connectTimeout: null` отключает
типизированное ограничение подключения. Автоматического возврата к native
транспорту при ошибке настроек нет.
...
Типизированное создание проверяет, что ответ содержит по одному уникальному
server ID на каждую отправленную строку. Переданные в ответе client ID должны
принадлежать запросу и не повторяться. Legacy-ответ без client ID остаётся
допустимым, но SDK не угадывает соответствия по порядку строк:
`serverIdForClientId()` для них вернёт `null`.

Неполное или некорректное подтверждение вызывает `UnconfirmedWriteException`
(подкласс `AmbiguousMutationException`). В нём сохранены `payloads` и исходный
`response` для сверки; автоматического повтора нет. Для этого исключения
`endpoint=null`, поэтому сначала проверь журнал чтением. Обновления по
известному server ID допускают пустой legacy-ответ и требуют отдельного чтения,
если нужно проверить сохранённые поля.
...
`UnconfirmedWriteException` также попадает в этот catch; его `endpoint` не
известен, а исходные `payloads` и `response` доступны для сверки результата.
...
- Credential-shaped методы покупок и чеков можно вызвать через `raw()`, но они
  пока не типизированы. Регистрация пользователя и некоторые внутренние методы
  сервера имеют другую сигнатуру credentials и намеренно не проксируются этим
  клиентом. Статус подписки уже доступен через `account()`.
