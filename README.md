...
Известное ограничение native PHP `SoapClient`: `readTimeout` передаётся в
`stream_context`, но не ограничивает ожидание самого SOAP-ответа. Для него PHP
использует `default_socket_timeout` окружения; SDK эту глобальную настройку не
меняет. Не рассматривай `readTimeout` как гарантированный лимит длительности
SOAP-вызова.
